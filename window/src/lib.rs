...
impl WindowTree {
    ...
    pub fn update(&mut self, wait: bool, render_state: &mut dyn State) -> Result<Option<Event>, Errno> {
        if let Some(cursor) = self.cursor {
            let (invalidated, screen_size) = self.invalidated();
...
mod tests {
    ...
    fn window_tree_new_window() {
        ...
        let one = Window::new(tree, None, None, Rect { tl: Point { x: 0, y: 0 }, size: Vector::null() });
        ...
        let two = Window::new(tree, None, Some(one), Rect { tl: Point { x: 0, y: 0 }, size: Vector::null() });
        ...
        let three = Window::new(tree, None, Some(two), Rect { tl: Point { x: 0, y: 0 }, size: Vector::null() });
        ...
        let four = Window::new(tree, None, None, Rect { tl: Point { x: 0, y: 0 }, size: Vector::null() });
